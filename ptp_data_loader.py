...
from CaptionDataset import CaptionDataset
import pandas as pd
import base64
import csv
import sys
...
class CocoDataset(data.Dataset):
    ...
    def __init__(self, root, json, concepts, vocab, transform=None, caption_dataset=None):
        ...
        Args:
            ...
            caption_dataset: instance of CaptionDataset for extended captions.
        """
        self.root = root
        self.coco = COCO(json)
    ...
    def __getitem__(self, index):
        ...
        if self.caption_dataset is not None:
            extra_caption = self.caption_dataset.get_next_caption(str(img_id))
...
def get_loader( root, json, concepts, vocab, transform, batch_size, shuffle, num_workers):
    ...
    with open('./data/my_caption.json', 'r', encoding='utf-8') as f:
        json_data = JSON.load(f)
    for item in json_data:
        caption_dataset.add_data(item["image_id"], item["caption"])
